 ...
 printf("# ============================================================================================");
   if ( test=='T' ) printf("======="); printf("\n");
     printf("# Driver for the evaluation of uGEMM\n");
       printf("# ============================================================================================");
         if ( test=='T' ) printf("======="); printf("\n");
	   printf("#      library resident m     n     k     mr     nr     kr     mc     nc     kc     Time   GFLOPS     Error");
        ...
        if (strcmp(variant,"BLIS\0") == 0)
	{
	   ...
	   GFLOPS  = flops / (1.0e+9 * time );
	    ...
	    while ( time <= tmin ) {
	      ...
	      int ldC = M;
	      M = m; N = n; K = k;
	      //gemm_microkernel_Cresident_neon_8x12_fp32( 'C', M, N, K, alpha, (one)?A:A2, B, beta, C, ldC );
	...
	//printf("M = %d, N = %d, KC = %d, PACKMR = %d PACKNR = %d. RoW? %d\n",
	//		M,N,PACKKC,PACKMR,PACKNR,row_pref);fflush(stdout);


	//printf("Test %s\n", (check_error(C,Cr,M*N))? "FAIL" : "OK"); 
	//printf("FIN\n");fflush(stdout);
}
